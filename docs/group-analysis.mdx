...
The analysis can include a maximum of 25 samples. The analysis results are presented in SNV Viewer with
...
or two parents without a proband). May include 2 or 3 samples. The analysis results are presented in SNV Viewer
with all variants discovered in the samples included in the analysis. SNV Viewer has specialized filters based
...
The names of the samples added to analysis must be unique. If you want to add the same samples to
your analysis, rename one of them (you can do this on [the sample page](/results/main/main#sample-info)),
so that the names are not repeated.
...
If you would like to receive notifications when the group analysis processing is complete, please enable
...
5. **Store annotated variants for [SNV Viewer](/group-analysis#group-analysis-variant-viewer):**
...
For germline cohort and family analyses, the results look like this (includes SNV Viewer):
...
- **SNV Viewer** is a embedded service for viewing and analyzing variants.
You can read more about it [below](/group-analysis#group-analysis-variant-viewer).
SNV Viewer is not provided for population analysis.
Click on <img src={require('/img/results/vv_btn.png').default} width="125"/> to open SNV Viewer.
...
#### Group Analysis SNV Viewer {#group-analysis-variant-viewer}

SNV Viewer is only provided for germline cohort and family analyses.
It is described in detail in [the corresponding section](/results/main/snvs-indels/variant-viewer) on
Sample SNV Viewer, however, Group Analysis SNV Viewer has a number of special features:
...
analysis SNV Viewer are germline.
