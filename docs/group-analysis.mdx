...
In "Group Analysis" section, you can perform ***germline cohort analysis*** and ***family analysis***.
Group analysis is a combining of germline variants (SNVs/Indels) discovered in samples selected for analysis.
Typically, germline cohort analysis is used to combine and compare samples that
are related in some way (for example, by pathology type or data source). Family group analysis is
for combining and comparing samples from the same family (proband + one or two parents or two parents without
a proband). Group analysis includes combining, filtering and annotating of germline variants of the samples.
...
At the bottom of the window, there is a table with samples for which group analysis can be performed.
The table includes the sample name and the patient code or run name to which it refers. A sample of
...
If you have selected ***more than three samples*** for analysis,
click on <img src={require('/img/eng/create_btn.png').default} width="130"/> to start analysis.

If you have selected ***two or three samples***,
click on <img src={require('/img/group_analysis/go_on_btn.png').default} width="130"/>:

<p align="center">
<img src={require('/img/group_analysis/all_chosen.png').default} height="600"/>
</p>

You will see the family analysis settings page:

<p align="center">
<img src={require('/img/group_analysis/family_settings.png').default} width="600"/>
</p>

If you want to perform a germline cohort analysis, turn off "Family Analysis" toggle button and
...
If you want to perform a family analysis, define for each sample the "family role" of the patient
from whom it was obtained: Proband (child), Father or Mother, and then
click on <img src={require('/img/eng/create_btn.png').default} width="130"/>:
...
- ANALYSIS TYPE: Germline cohort or Family. Can be changed by clicking on the value field and selecting
a different type of analysis (if the samples used for analysis have not been deleted).
...
2. Perform Joint Genotyping of analysis samples
using [GATK GenotypeGVCFs](https://gatk.broadinstitute.org/hc/en-us/articles/360037057852-GenotypeGVCFs) tool;
3. Filter Germline SNVs/Indels: filtering variants
...
- Text file in **CSV** (Comma-Separated Values) format. To download the file,
click on <img src={require('/img/results/table_btn.png').default} width="120"/>.
- File in **[VCF 4.2 (Variant Call Format)](http://samtools.github.io/hts-specs/VCFv4.2.pdf)** format.
To download the file, click on <img src={require('/img/results/vcf_btn.png').default} width="45"/>.
- File in **Google Spreadsheet** (only a limited number of variants can be shown). To open the file,
...
Group Analysis Variant Viewer is almost the same as [Sample Variant Viewer](/results/main/snvs-indels/variant-viewer),
...
are designated by certain patient "family roles" (when you hover over the column name, you can see the full name
of the sample):
...
using ["Trio GT (Genotype)" filter](/results/main/snvs-indels/advanced-filtering#multiselect-filters).

3. There is no column, basic and advanced filter "Origin", since all variants are germline.
