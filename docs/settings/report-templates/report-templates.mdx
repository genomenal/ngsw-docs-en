...
**Editable fields:**
1. *Name* – section title (required field).
If there are multiple sections in the report, they are numbered automatically.
2. *Description* – text displayed below the section title.
...
- `{currentDate}` - current date in the format "M d, yyyy", can be used as the report issue date.
...
4. *Interpretation* – defines how interpretations are included in the report:
- *By variants* (default) – only interpretations added for each individual genetic variant included in the
report are displayed.
- *Overall for the block* – only a single interpretation for the entire block is displayed; it appears below
the table of genetic variants. When this option is selected, the following settings become available:
   - *Transfer variant interpretation to overall interpretation* – when enabled, interpretations added for genetic
   variants included in the report will also be displayed in the overall interpretation section.
   - *Interpretation text* – a field for entering the overall interpretation; this text can be edited directly
   in the report.
- *By variants and overall for the block* – both individual variant interpretations and an overall interpretation
are displayed in the report. When this option is selected, the following settings become available:
   - *Transfer variant interpretation to overall interpretation* – when enabled, interpretations added for genetic
   variants included in the report will also be displayed in the overall interpretation section.
   - *Interpretation text* – a field for entering the overall interpretation; this text can be edited directly
   in the report.
...
2. *Description* – text displayed below the section title.
3. *Origin* – somatic or germline.
4. *Conditions for including variants to the report* - a rule that defines which variants will be displayed in
this report section. If the variants discovered in a sample meet the specified conditions, they are included
in this section. To define a condition, click
the button <img src={require('/img/report_templates/query_builder.png').default} width="170"/> - the query
builder will open. A detailed description of how to use the query builder is provided
in [the relevant section](/results/main/snvs-indels/advanced-filtering#add-condition).
The query builder largely replicates the advanced variant search form on the SNV Viewer page, with the
following limitations:
- It is not possible to select multiple saved queries simultaneously (if needed, conditions can be combined
using the OR operator);
- The compound heterozygote filter is not available.
5. *Interpretation* – defines how interpretations are included in the report:
- *By variants* (default) – only interpretations added for each individual genetic variant included in the
report are displayed.
- *Overall for the block* – only a single interpretation for the entire block is displayed; it appears below
the table of genetic variants. When this option is selected, the following settings become available:
...
are displayed in the report. When this option is selected, the following settings become available:
   ...
   - *Interpretation text* – a field for entering the overall interpretation; this text can be edited directly
   in the report.
6. *Use SNVs/Indels as potential findings*: To include SNVs/Indels detected in the sample that meet the
...
:::note About Log2FC Thresholds
The variation filter by log2FC thresholds (the logarithmic ratio of the observed copy number to the normal
copy number), offers the following options:
...
Let’s take threshold *Del: ≤-0.7 (chrX: ≤-2.11, chrY: ≤-2.11), Dup: ≥0.46 (chrX: ≥0.82, chrY: ≥0.82) (XY) Del: ≤-0.7 (chrX: ≤0.3), Dup: ≥0.46 (chrX: ≥1.46, chrY: ≥-0.37) (XX)*
as an example to explain how these filtering conditions work:
- If the sample's chromosomal sex is determined\* as male (XY), the variations meeting the following threshold
will be included in the corresponding report
blocks: *Del: ≤-0.7 (chrX: ≤-2.11, chrY: ≤-2.11), Dup: ≥0.46 (chrX: ≥0.82, chrY: ≥0.82)*.
If the sample's chromosomal sex is determined\* as female (XX), the variations meeting the following threshold
will be included in the corresponding report
blocks: *Del: ≤-0.7 (chrX: ≤0.3), Dup: ≥0.46 (chrX: ≥1.46, chrY: ≥-0.37)*.
- CNVs on autosomes with log2FC ≤ -0.7 are classified as deletions.
A log2FC below –0.7 corresponds to detecting one autosomal copy instead of two in the normal genome.
- CNVs on autosomes with log2FC ≥ 0.46 are classified as duplications.
The threshold for duplications is calculated as follows: *log2FC = log<sub>2</sub>((2+D)/2)*, where *D* is the copy
number "lost" at log2FC=-0.7, and *2* is the normal number of sister chromatids in autosomes.
- CNVs on the X chromosome are classified as deletions if log2FC ≤ -2.11 for XY samples
and log2FC ≤ 0.3 for XX samples; and as duplications if log2FC ≥ 0.82 for XY samples and log2FC ≥ 1.46 for XX samples.
- CNVs on the Y chromosome are classified as deletions if log2FC ≤ -2.11 for XY samples;
and as duplications if log2FC ≥ 0.82 for XY samples and log2FC ≥ -0.37 for XX samples.
- Variations not meeting the thresholds - i.e., autosomal variations with -0.7 < log2FC < 0.46, X-chromosomal
variations with -2.11 < log2FC < 0.82 for XY samples or 0.3 < log2FC < 1.46 for XX samples, and Y-chromosomal
variations with –2.11 < log2FC < 0.82 for XY samples or log2FC < –0.37 for XX samples - do not pass the filter
and are excluded from the report.

*The sample's chromosomal sex is determined by CNVkit from the relative copy number of the autosomes and sex
chromosomes.
:::

5. *Interpretation* – defines how interpretations are included in the report:
- *By variations* (default) – only interpretations added for each individual CNV included in the
report are displayed.
- *Overall for the block* – only a single interpretation for the entire block is displayed; it appears below
the table of CNVs. When this option is selected, the following settings become available:
   - *Transfer variation interpretation to overall interpretation* – when enabled, interpretations added for CNVs
   included in the report will also be displayed in the overall interpretation section.
   - *Interpretation text* – a field for entering the overall interpretation; this text can be edited directly
   in the report.
- *By variations and overall for the block* – both individual CNV interpretations and an overall interpretation
are displayed in the report. When this option is selected, the following settings become available:
   - *Transfer variation interpretation to overall interpretation* – when enabled, interpretations added for CNVs
   included in the report will also be displayed in the overall interpretation section.
   - *Interpretation text* – a field for entering the overall interpretation; this text can be edited directly
   in the report.
6. *Use CNVs as potential findings*: To include copy number variations detected in the sample that meet the
specified conditions as [potential findings](/results/potential-findings), activate the toggle switch.
...
are displayed in the report. When this option is selected, the following settings become available:
   ...
   - *Interpretation text* – a field for entering the overall interpretation; this text can be edited directly
   in the report.
6. *Use the most important variants for interpretation as potential findings*:
...
7. *Use the variants that require further analysis as potential findings*:
...
### 26. Pinned CNVs {#pinned-cnv}

A block containing the copy number variations pinned in the CNV Viewer table.
Pinning variations is described in [the corresponding section](/results/main/copy-number-variations/cnv-viewer#pin-variation).
...
**Display of pinned CNVs in the report:**
- If a full region is pinned, it is displayed in the report without information about affected genes.
- If one or more split regions are pinned, the report displays the corresponding full region, while the
"Affected genes" column includes only the genes corresponding to the pinned split records within that region.
- Only the interpretation added for the full region is displayed in the report.
Interpretations added for split regions are not displayed.

### 27. Detection of whole-chromosome and сhromosome arm-level aneuploidies: results {#cnv-results}
...
### 28. Detection of whole-chromosome and сhromosome arm-level aneuploidies: interpretation {#cnv-interpretation}
...
### 29. Onco: Onco relevant SNVs/Indels selected by user for reports {#onco-variants}

The block displays a table with information about clinically
significant [onco-relevant variants](/results/main/snvs-indels/onco-relevance) included
in the report from the SNV Viewer page.<br></br> Including variants in the report is described
...
:::caution
The block is intended for use only with onco-relevant variants.
If a variant included in the report is not onco-relevant, it will not appear in this block.
:::
...
### 30. Onco: Clinical Guidelines for onco relevant SNVs/Indels selected by user for reports {#clinical-recommendations}
...
### 31. Polygenic traits {#polygenic-traits}
...
- Alzheimer's Disease;
- Parkinson's Disease;
- Prostate cancer (displayed in the report if the patient's sex is determined to be male);
- Breast cancer (displayed in the report if the patient's sex is determined to be female);
- Colorectal cancer;
- Neuroticism Score;
- Risk-Taking Behaviour;
- Risk of hospitalization for COVID-19;
- Risk of very severe COVID-19.
4. *Add interpretation block* – a toggle that enables the display of an overall interpretation for the
entire block in the report. When this toggle is enabled, the *Interpretation text* field becomes available. This
field is used to enter the overall interpretation, and the entered text can be edited directly within the report.

**Example block in the report:** [Report on Polygenic Traits](/results/polygenic-report).

### 32. SNVs/Indels by ClinVar phenotypes {#clinvar-phenotype}
...
### 33. Pharmacogenetic {#pharmacogenetics}
...
1. *Name* – section title (required field).
2. *Description* – text displayed below the section title.
3. *Add interpretation block* – a toggle that enables the display of an overall interpretation for the
entire block in the report. When this toggle is enabled, the *Interpretation text* field becomes available. This
field is used to enter the overall interpretation, and the entered text can be edited directly within the report.

### 34. Oligogenic traits {#oligogenic-traits}

The block displays results of oligogenic trait prediction. Oligogenic traits are characteristics whose manifestation
depends on a small number of genes, each making a substantial contribution to the phenotype.
...
1. *Name* – section title (required field).
2. *Description* – text displayed below the section title.
3. *Oligogenic traits* - a selection of traits whose prediction results will be shown in the report:
- Hair color;
- Eyes color;
- Skin color;
- Freckling;
...
**Example block in the report:** [Report on Oligogenic Traits](/results/oligogenic-report).

### 35. SNP panel interpretation by genotype {#snp-panel-genotypes}

This block displays the polymorphisms added to the SNP panel together with the interpretation defined
for each genotype.
...
1. *Name* – section title (required field).
2. *Description* – text displayed below the section title.
3. *Selected SNPs* - a table containing the polymorphisms added to the SNP panel.

Example of an SNP panel:

<p align="center">
<img src={require('/img/eng/selected_snps_example.png').default} width="900"/>
</p>

The table columns correspond to the fields in the SNP addition form:
   - *Gene* - the name of the gene in which the polymorphism is located. If gene information is unavailable in
   the database, a dash (`-`) is displayed.
   - *rsID* - the polymorphism ID from the <a href="https://www.ncbi.nlm.nih.gov/snp/">dbSNP</a> database,
   displayed as a link to the corresponding page. The value is specified by the user.
   - *Alteration* - displayed according to the selected allele/genotype combination:
       - `reference allele > alternative allele` - for alternative homozygous and heterozygous genotypes;
       - `reference allele > NON_REF`, where NON_REF denotes any allele other than the reference allele, - for
       the reference heterozygous genotype;
       - `reference allele =` - for the reference homozygous genotype. This notation indicates that no
       substitution occurs and the variant is represented by the reference allele.
   - *Genotype* - displayed according to the selected allele/genotype combination:
       - `reference allele / alternative allele` - for the alternative heterozygous genotype;
       ...
       - `reference allele / NON_REF`, where NON_REF denotes any allele other than the reference allele, - for
       the reference heterozygous genotype.
   - *Exon* - the exon number of the transcript in which the polymorphism is located.<br></br>
   For alternative homozygous, alternative heterozygous, and reference heterozygous genotypes, this field
   is always left empty because exon information is taken from the genetic variant annotation and added to
   the final report.<br></br>
   For the reference homozygous genotype, the value is retrieved from the database or specified by the user.<br></br>
   The supported format is `exon number (or exon range) / total number of exons in the transcript`, for example
   3/8 or 3-4/8. If the information is unavailable, a dash (`-`) is displayed.
   - *Interpretation* - a textual interpretation of the selected genotype. The value is specified by the
   user and displayed in the final report.

SNPs of the same gene are displayed together in the order in which they were added to the panel.
When a new entry is added for a gene already represented in the panel, it is displayed after the previously
added entries for that gene.

4. *Coverage threshold for genotypes* - the minimum sequencing depth at the polymorphism position required for
the genotype to be included in the final report. The default value is 20, meaning that the position must be
covered by at least 20 reads.
5. *Add interpretation block* – a toggle that enables the display of an overall interpretation for the
entire block in the report. When this toggle is enabled, the *Interpretation text* field becomes available. This
field is used to enter the overall interpretation, and the entered text can be edited directly within the report.

**Adding an SNP to the panel:**

To add a polymorphism to the SNP panel,
click <img src={require('/img/report_templates/add_snp.png').default} width="190"/> and fill in the fields
in the dialog:
1. *rsID* - the polymorphism ID from [the dbSNP database](https://www.ncbi.nlm.nih.gov/snp/). It can be
entered either as `rs12345` or simply as `12345`.<br></br>
If the specified identifier is not found in the database, the message "*Cannot find rsId in DbSNP*" is displayed,
and the polymorphism cannot be added.<br></br>
If the ID is recognized successfully, the Allele and Interpretation fields become available.
If gene information is available for the polymorphism, the "*Gene*" field is also displayed.

<p align="center">
<img src={require('/img/report_templates/add_snp_form.png').default} width="600"/>
</p>

2. <i>Allele</i> - selects one of the variant alleles: the reference allele or one of the alternative
alleles (if multiple alternative alleles exist, all of them are presented). Each allele is labeled as
either reference or alternative.

<p align="center">
<img src={require('/img/report_templates/alleles_options.png').default} width="600"/>
</p>

After an allele is selected, the Genotype field becomes available.

3. <i>Genotype</i> - selects either a homozygous or heterozygous genotype.

The following genotype combinations can be added for each SNP:
   - *Reference homozygous* = reference allele + homozygous genotype.
   Selecting this combination displays the additional Exon field.
   - *Reference heterozygous* = reference allele + heterozygous genotype. This entry represents all possible
   heterozygous genotypes with alternative alleles. Once added, individual heterozygous genotypes for
   alternative alleles cannot be added.
   - *Alternative homozygous* = alternative allele + homozygous genotype.
   If multiple alternative alleles exist, a separate entry can be added for each allele.
   - *Alternative heterozygous* = alternative allele + heterozygous genotype.
   If multiple alternative alleles exist, a separate entry can be added for each allele.
   This combination is mutually exclusive with the reference heterozygous genotype: for a given SNP, you
   can add either the reference heterozygous genotype or one or more alternative heterozygous genotypes.

4. *Exon* - a field displayed only for the reference homozygous genotype.<br></br>
The supported format is `exon number/total number of exons` or `exon range/total number of exons`,
for example 1/3 or 1-2/3. The values refer to the primary transcript of the gene.<br></br>
If exon information is available in the database, the field is filled automatically but can be edited
if necessary.<br></br>
If the information is unavailable, the field remains empty but is still required. Enter the exon number
in the supported format, or enter a dash (`-`) if the variant is not located within an exon or the exon
number is unknown.

5. <i>Interpretation</i> - a textual interpretation of the selected genotype that will be displayed in the
final report. The field supports <a href="https://www.markdownguide.org/basic-syntax/">Markdown formatting</a>.

After completing all required fields, click <img src={require('/img/custom-annot/ok_btn.png').default} width="130"/>.
The polymorphism is then added to the table.

While adding or editing the block, you can edit or remove a polymorphism entry by
clicking <img src={require('/img/custom-annot/edit_column.png').default} width="25"/>
or <img src={require('/img/eng/delete_btn_white.png').default} width="25"/>, respectively.

**Example block in the report:** [SNP Panel Genotype Interpretation Report](/results/snp-panel-report).

### 36. Ancestry {#ancestry}

The report presents the results of a genetic ancestry analysis, which is performed by
[ADMIXTURE](https://genome.cshlp.org/content/19/9/1655) using
...
entire block in the report. When this toggle is enabled, the *Interpretation text* field becomes available. This
field is used to enter the overall interpretation, and the entered text can be edited directly within the report.
4. *Probability threshold for populations* - the minimum threshold for the membership probability obtained
from the ancestry analysis. Only populations for which the calculated probability - reflecting the contribution
of that population to the genetic structure of the sample - exceeds the specified threshold are included in the
report. The recommended default value is 0.01.

**Example block in the report:** [Ancestry Report](/results/ancestry-report).
