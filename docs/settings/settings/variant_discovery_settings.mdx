...
#### Disable limit reads per alignment
...
### SNVs/Indels filtration

#### Filter SNVs/indels by Capture Kit target and disable it
...
#### Allow only variants passing all filters and disable it
...
and displayed in the Variant Viewer.
If this option is disabled, then all variants are displayed in the Variant Viewer
...
:::note
This option only affects the display of unfiltered variants in the Variant Viewer and Bioinformatic report.
In further analysis (for example, in a group analysis), only filtered variants will still be accepted.
:::

### Somatic SNVs/indels discovery
...
#### Disable somatic SNVs/indels discovery
...
somatic SNV/indels discovery in the gold standard sample (breast cancer sequencing sample SRR7890918
...
- *None* - select this option if you want to perform somatic SNV/indels detection without a PON.

To change the PON for somatic SNV/indels discovery, click on the value of the "Panel of normals"
...
#### Disable somatic tumor only discovery
...
### Germline SNVs/indels discovery
...
#### Disable germline SNVs/indels discovery
...
#### Disable germline tumor only discovery
...
#### The germline SNV/indels hard-filtering parameters
...
#### Disable or enable hard-filtering parameters and change their thresholds
...
To enable CNN scoring of germline SNV/indels, turn on the corresponding toggle:
...
#### Disable structural variations discovery
...
#### Disable copy number variations discovery
...
#### Choose the minimal bin size for WGS data
...
#### Call germline aneuploidies only
...
#### Select reference samples panel
...
genotyping data in GT format or annotated germline SNV/indels in VCF format.

#### Enable phenotype prediction
...
#### Disable quality control and imputation
