...
## Data filtration for variant discovery {#filtration-for-variant-discovery}
...
### Mark Duplicates {#disable-mark-duplicates}
...
:::tip
If you are analyzing amplicon panel data, we recommend *disabling* mark duplicates to improve SNVs/indels calling.
:::

### Limit reads per alignment {#disable-limit-reads}
...
:::tip
If you are analyzing amplicon panel data, we recommend *disabling* limit reads per alignment
to improve germline SNVs/Indels calling.
:::

### Using soft clipped bases {#soft-clipped-bases}

Soft-clipped bases are bases located at the 3' and 5' ends of a read that do not align to the reference sequence.
They are excluded from the alignment itself; however, unlike hard-clipped bases, they are not discarded during
alignment and are retained for downstream analysis by variant calling tools such
as ***[GATK Mutect2](https://gatk.broadinstitute.org/hc/en-us/articles/360037593851-Mutect2)***
and ***[GATK HaplotypeCaller](https://gatk.broadinstitute.org/hc/en-us/articles/360037225632-HaplotypeCaller)***.
Although the use of soft-clipped bases may increase the risk of false-positive calls, excluding them from
analysis is generally not recommended. In many cases, soft-clipped bases provide the only evidence of true
insertions and deletions (indels). If multiple reads are soft-clipped at the same position, this may indicate
the presence of a structural variant.

Disabling this feature is recommended only in cases where bases that should have been hard-clipped at earlier
stages of the pipeline were instead soft-clipped. To exclude soft-clipped bases from the analysis, enable
the corresponding toggle:

<p align="center">
<img src={require('/img/eng/soft_clipped_bases.png').default} width="400"/>
</p>

:::tip
If you are analyzing amplicon panel data, *enabling* excluding soft-clipped bases from the analysis
is recommended to improve variant calling.
:::

## SNVs/Indels filtration {#snvs-filtration}

### Filter SNVs/indels by Capture Kit target {#filter-snvs-by-capture-kit}
...
### Allow only variants passing all filters {#allow-filtered-variants}
...
### Somatic tumor only discovery {#disable-somatic-tumor-only-discovery}
...
### Filter strand and orientation bias {#filter-strand-orientation-bias}

This setting controls the filtering of sequencing artifacts associated with strand bias and read orientation bias.

By default, the setting is enabled. In this case, orientation bias priors are used during variant filtering,
allowing the removal of false-positive variants that exhibit bias toward a single read direction (forward or reverse).

When the setting is disabled, filtering of strand and orientation bias-related artifacts is not performed.
This results in the following changes to the pipeline:
- The variant caller GATK Mutect2 is not provided with the `--f1r2-tar-gz` argument used to collect F1R2 metrics.
F1R2 refers to a type of paired reads in which the first read aligns to the forward strand of the reference,
while the second read aligns to the complementary strand.
- The read orientation model is not constructed using GATK LearnReadOrientationModel, which estimates prior
probabilities of orientation bias artifacts using a maximum likelihood approach.
- The `--ob-priors` argument is not passed to GATK FilterMutectCalls. This argument contains the prior
probabilities of artifacts derived from the read orientation model.

Disabling orientation bias models may lead to the detection of additional variants, including those with low
variant allele fractions. This can be beneficial when analyzing amplicon panel data. However, it also
substantially increases the number of false-positive calls.

To disable filtering of sequencing artifacts related to strand and orientation bias, turn off the
corresponding toggle:

<p align="center">
<img src={require('/img/eng/filter_strand_bias.png').default} width="400"/>
</p>

:::tip
When analyzing amplicon panel data, it is recommended to disable sequencing artifact filtering to improve
detection of somatic SNVs/Indels.
:::
...
### Germline tumor only discovery {#disable-germline-tumor-only-discovery}
...
:::tip
If you are analyzing amplicon panel data, to improve germline SNVs/indels calling, we recommend adjusting
the hard-filtering parameters as follows:
- FS SNP parameter: disable or increase the value to 150-200;
- SOR SNP parameter: disable or increase the value to 8-10;
- MQ SNP parameter: decrease the value to 30;
- MQRankSum SNP parameter: decrease the value to -20;
- ReadPosRankSum SNP parameter: decrease the value to -12.
:::
...
### CNN variant scoring {#enable-cnn-scoring}
...
## Genomic predictions {#phenotype-prediction}

### Oligogenic risk scores calculation {#enable-phenotype-prediction}

Oligogenic traits are characteristics such as hair or eye color whose manifestation depends on a small
number of genes, each making a substantial contribution to the phenotype.
Calculation of oligogenic risk scores is based on single nucleotide polymorphisms (SNPs) associated with specific
trait manifestations, using models built on multinomial logistic regression (MLR). We use models that
predict such oligogenic traits as hair color, eyes color, skin color, freckling, lactose intolerance,
blood type, alcohol metabolism, body odour (axillary osmidrosis), earwax type, bitter taste receptor
sensitivity, misophonia, sleep movement risk, and photic sneeze reflex sensitivity.
Oligogenic risk scores calculation can only be performed on non-tumor samples:
genotyping data in GT format or annotated germline SNVs/indels in VCF format.

To enable oligogenic risk scores calculation in sample, turn on the corresponding toggle:

<p align="center">
<img src={require('/img/eng/turn_on_phenotypes.png').default} width="400"/>
</p>

### Polygenic risk scores calculation {#polygenic-risk-scores-calculation}
...
Polygenic risk scores calculation can only be performed on non-tumor samples.
...
The polygenic risk scores are calculated for the following traits:
   ...
   - Predisposition to hyperthyroiditis;
...
### Pharmacogenetic calculation {#pharmacogenetics}

Pharmacogenetic testing makes it possible to account for the patient's hereditary characteristics when
prescribing medications. Pharmacogenetic calculation is the analysis of genetic variants in a sample to
determine the alleles and diplotypes of pharmacogenetically significant genes using
a tool [PharmCAT](https://pharmcat.clinpgx.org/). The interpretation of the results is based on the data
from [CPIC](https://cpicpgx.org/),
[PharmGKB](https://www.pharmgkb.org/), [DPWG](https://www.clinpgx.org/page/dpwg), and [FDA](https://www.fda.gov/).
Pharmacogenetic calculation can only be performed on non-tumor samples.

To enable pharmacogenetic calculation in sample, turn on the corresponding toggle:

<p align="center">
<img src={require('/img/eng/pharma_setting.png').default} width="400"/>
</p>

### Ancestry analysis {#ancestry-analysis}

Ancestry inference is performed using
...
During the analysis, the genotype of the analyzed sample is projected onto the trained model, resulting in
estimated proportions of contribution from each ancestral component to the individual’s genome.
Ancestry analysis can only be performed on non-tumor samples.

To enable ancestry analysis in sample, turn on the corresponding toggle:

<p align="center">
<img src={require('/img/eng/ancestry_setting.png').default} width="400"/>
</p>

#### Number of ancestral components {#ancestral-components}

Number of ancestral components (K) used by ADMIXTURE to decompose the sample. Each ancestral component
represents a hypothetical genetic group characterized by specific allele frequency patterns and reflects
the structure of genetic variation present in the reference population data.
...
component to the individual’s genome.

By default, the analysis is performed using 10 ancestral components. The allowed range of values is from
2 to 20. The parameter value can be changed by clicking on the current value and editing it. To save changes,
press Enter or click outside edit value container.

<p align="center">
<img src={require('/img/eng/ancestral_components.png').default} width="400"/>
</p>

The number of ancestral components determines the level of detail of the ancestry analysis, that is, the
number of statistically distinguishable genetic clusters that the model attempts to identify in the reference data.

Effect of the number of ancestral components on the analysis:
- Low values of K (K = 2-5): the components typically correspond to large continental or superpopulation-level
clusters (e.g., Africa, Europe, America). Genetic differences within these groups are largely smoothed out.
The analysis is highly stable and easy to interpret, but it does not allow discrimination between closely
related populations.
- Intermediate values of K (K = 6-10): finer differentiation within continents emerges, reflecting population
structure and historical migration processes. The results become more informative, but their interpretation
requires greater care, as components may partially overlap between populations.
- High values of K (K = 11-20): components are split into narrow clusters, enabling the detection of subtle
differences between closely related populations. However, at high K values there is an increased risk that
components reflect not only biological structure but also characteristics of the reference panel, sample
size effects, or statistical noise. Results obtained at this level require cautious and expert interpretation.

### Quality control and imputation {#disable-qc-and-imputation}

Before running the oligogenic risk scores calculation, polygenic risk scores calculation and ancestry analysis,
three preparatory steps with quality control and imputation are carried out:
...
Imputation significantly increases the number of markers on the basis of which genomic predictions will occur.
3. Quality control.

These steps are important for the quality of genomic predictions, but are not always successful
