...
:::note
If you are on the page of [Variant Viewer with somatic mutations](/results/main/snvs_indels/variant-viewer),
...
## Adding the First Filter Condition
...
- The columns:
    1. **Gene** (the "Locus" column, the "Gene" basic filter): filtering by gene transcript (shown in brackets).
    ...
    [here](/results/main/snvs_indels/variant-viewer#impacts-of-variants-in-the-annotation).
    ...
    [here](/results/main/snvs_indels/variant-viewer#consequences-of-variants-in-the-annotation).
    5. **Pathogenicity**: filtering by variant pathogenicity, which have been determined either manually, by ACMG
    criteria, or according to the pathogenicity base.
    6. **Chromosome** (the "Position" column): filtering by the chromosome in which the variant is located: chr1-chr22,
    ...
    7. **Origin**: filtering by mutation type: Somatic, Germline.
    8. **Caller Filters**: filtering by GATK variant filters.
    9. **ClinVar: Clinical significance** (the "ClinVar" column/basic filter): filtering by the clinical
    ...
    10. **ClinVar: Allele origin**: filtering by variant allele origin.
    11. **Conservation: [conservation score] confidence value**: filtering by confidence value of various conservative
    scores (Integrated, GM12878, H1-hESC, HUVEC): Highly significant; Significant; Informative; Other.
    12. **Protein function effect: [prediction algorithm] prediction**: filtering by the results of predicting
    ...
    13. **Other: APPRIS annotation** filtering by human splicing isoforms
...
- The filters:
    ...
    detail in [the corresponding section](/settings/gene_panels). In addition, it is possible to import panels from
    the Library of the most common clinical panels - such option is available in the "Gene panel" filter in the
    [Basic Filtering](/results/main/snvs_indels/basic_filtering) mode. If you don't have any gene panels added,
...
- The columns:
    1. **rsId** (the "External links" column): filtering by variant ID from
    the [dbSNP](https://www.ncbi.nlm.nih.gov/snp/) database. You can use as a value either the full ID (rs\*\*\*\*\*\)
    or the ID digits only (\*\*\*\*\*\).
    2. **ClinVar ID** (the "External links" column): filtering by variant ID (Variation ID) from
    the [ClinVar](https://www.ncbi.nlm.nih.gov/clinvar/) database.
    3. **GnomAD ID**: filtering by variant ID from the [gnomAD](https://gnomad.broadinstitute.org/) database.
    ...
    according to the [ClinVar](https://www.ncbi.nlm.nih.gov/clinvar/) database.
...
- The columns:
    ...
    4. **GnomAD: Allele frequency** (the "gnomAD AF" column): filtering by the total allele frequency in the
    [gnomAD](https://gnomad.broadinstitute.org/) database (positive rational number).
    5. **GnomAD: XY samples AF**: filtering by allele frequency in the XY subpopulation in the
    [gnomAD](https://gnomad.broadinstitute.org/) database (positive rational number).
    6. **GnomAD: XX samples AF**: filtering by allele frequency in the XX subpopulation in the
    [gnomAD](https://gnomad.broadinstitute.org/) database (positive rational number).
    7. **GnomAD: Number of Homozygotes**: filtering by the number of homozygotes in the
    [gnomAD](https://gnomad.broadinstitute.org/) database (natural number).
    8. **GnomAD: [population] AF / XY/XX samples AF / Number of Homozygotes**: filtering by total allele frequency (AF),
    by allele frequencies in XX/XY subpopulations or by the number of homozygotes in different populations (Ashkenazi
    Jewish, Amish, African and African American, Finnish, South Asian, Latino, Non-Finnish european, East Asian, Middle
    Eastern, Other) in the [gnomAD](https://gnomad.broadinstitute.org/) database (positive rational number).
    9. **Conservation: [prediction program] rankscore / score / neutral value**:
    filtering by conservation scores obtained using prediction by various programs
    ...
    10. **Conservation: [ pA / pG / pC / pT ]**: filtering by stationary distribution of alleles A, G, C or T in
    the site, estimated using the [SiPhy](http://portals.broadinstitute.org/genome_bio/siphy/documentation.html)
    algorithm (positive rational number).
    11. **ExAC: Allele frequency**: filtering by the total allele frequency in the
    [ExAC](http://exac.broadinstitute.org/) database (positive rational number).
    12. **ExAC: Allele count**: filtering by alternative allele count in the
    [ExAC](http://exac.broadinstitute.org/) database (natural number).
    13. **ExAC: Adjusted AF**: filtering by allele frequency in a population with genotype quality ≥20 and
    depth ≥10 in the [ExAC](http://exac.broadinstitute.org/) database (positive rational number).
    14. **ExAC: Adjusted allele count**: filtering by alternative allele count in a population with genotype
    quality ≥20 and depth ≥10 in the [ExAC](http://exac.broadinstitute.org/) database (natural number).
    15. **ExAC: [population] AF / allele count**: filtering by total allele frequency (AF) or
    by alternative allele count in different populations (African & African American, American, East Asian,
    Finnish, Non-Finnish European, South Asian) in the [ExAC](http://exac.broadinstitute.org/) database
    (positive rational number).
    16. **ExAC: [cohort] AF / Allele count / adjusted AF / adjusted allele count**: filtering by total allele
    frequency (AF), by alternative allele count, or by allele frequency or alternative allele count in
    a population with genotype quality ≥20 and depth ≥10 in different cohorts (Non-TCGA, Non-Psych) in the
    [ExAC](http://exac.broadinstitute.org/) database (positive rational number for AF and natural number for
    allele count).
    17. **ExAC: \[cohort] \[population] AF / allele count**: filtering by total allele frequency (AF) or
    by alternative allele count in different populations (African & African American, American, East Asian,
    Finnish, Non-Finnish European, South Asian) in different cohorts (Non-TCGA, Non-Psych) in the
    [ExAC](http://exac.broadinstitute.org/) database (positive rational number for AF and natural number
    for allele count).
    18. **Protein function effect: [prediction algorithm] rankscore / score / omega / p-value**: filtering by
    prediction scores of the effect of amino acid substitution on protein function, obtained using various algorithms
    and programs ([SIFT](https://sift.bii.a-star.edu.sg/),
    [Polyphen](http://genetics.bwh.harvard.edu/pph/pph_help.html),
    ...
    (positive rational number).
    19. **Other frequencies: 1000G Allele frequency**: filtering by total allele frequency in
    [the 1000 Genomes Project](https://www.internationalgenome.org/) (positive rational number).
    20. **Other frequencies: 1000G [population] AF / allele count**: filtering by total allele frequency or
    by alternative allele count in different populations (East Asian, European, African, American, South Asian,
    African American, European American) in [the 1000 Genomes Project](https://www.internationalgenome.org/)
    (positive rational number for AF and natural number for allele count).
    21. **Other frequencies: UK10K allele frequency**: filtering by total allele frequency in combined genotypes
    in the UK10K cohort from [the UK10K project](https://www.uk10k.org/) (positive rational number).
    22. **Other frequencies: UK10K allele count**: filtering by alternative allele count in combined genotypes
    in the UK10K cohort from [the UK10K project](https://www.uk10k.org/) (natural number).
    23. **Other frequencies: UK10K [cohort] AF / allele count**: filtering by total allele frequency or by
    alternative allele count in combined genotypes in different cohorts (TwinsUK, ALSPAC) from
    [the UK10K project](https://www.uk10k.org/) (positive rational number for AF and natural number for allele count).
    24. **Other: Coding sequence position**: filtering by coding sequence position (natural number).
    25. **Other: Protein position**: filtering by protein position (natural number).
    26. **Other: Amino acid position**: filtering by amino acid position (natural number).

### 5. Filters with any value
...
- The columns:
    ...
    6. **Common: RefSeq Transcript**: filtering by transcript ID from the
    [RefSeq](https://www.ncbi.nlm.nih.gov/nuccore/) database (NM_\*\*\*\*\*\.\*\).
    7. **GnomAD: Coverage**: filtering by average coverage depth by bases
    in the [gnomAD](https://gnomad.broadinstitute.org/) database: &lt;10, 10-100, ⩾100.
    8. **ClinVar: Phenotypes**: filtering by the phenotypic significance of the variant according to the
    ...
    of the haplotype, according to the [ClinVar](https://www.ncbi.nlm.nih.gov/clinvar/) database.
    ...
    18. **Protein function effect (additional): Transcript ID**: filtering by transcript ID from the
...
"[Per-sample data: Genotype](/results/main/snvs_indels/advanced_filtering#8-filter-by-genotype)"):
...
### 9. Filters by custom annotation columns
...
### The searched variant match both conditions
...
in [the corresponding section](/results/main/snvs_indels/advanced_filtering#adding-the-first-filter-condition).
...
### The searched variant match one of the conditions
...
In addition, the saved query will appear in [the Basic Filtering mode](/results/main/snvs_indels/basic_filtering).
