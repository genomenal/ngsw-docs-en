...
- The columns:
    ...
    7. **Chromosome** (the "Position" column): filtering by the chromosome in which the variant is located: chr1-chr22,
    ...
    8. **Origin**: filtering by mutation type: Somatic, Germline.
    9. **Caller Filters**: filtering by GATK variant filters.
    10. **ClinVar: Clinical significance** (the "ClinVar" column/basic filter): filtering by the clinical
    ...
    11. **ClinVar: Allele origin**: filtering by variant allele origin.
    12. **Conservation: [CS] confidence value**: filtering by confidence value of various conservative
    scores: Highly significant; Significant; Informative; Other,
    where **[CS]** is a conservation score: Integrated, GM12878, H1-hESC, HUVEC.
    13. **Protein function effect: [PA] prediction**: filtering by the results of predicting
    ...
    14. **Other: APPRIS annotation**: filtering by human splicing isoforms
    ([APPRIS](http://appris.bioinfo.cnio.es) annotation): Principal 1-5; Alternative 1-2.
    15. **Trio GT (Genotype)**: filtering by variant inheritance: *De novo* variants, Newly formed
