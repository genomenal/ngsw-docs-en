...
1. [Somatic and Germline SNVs/Indels](/results/main/snvs_indels/snvs_indels)
2. [Somatic and Germline Structural Variations](/results/main/structural_variations)
...
- SEQUENCING TYPE: Panel (sequencing using a targeted panel), WES (whole-exome sequencing), WGS (whole-genome
sequencing), Low-pass WGS (whole-genome sequencing with low coverage), UNKNOWN (the sequencing type may not be
recognized for some samples uploaded in VCF format). Determined automatically, can be based on the sequencing type
and Capture Kit that were selected while adding sample set. Can be changed on [the Parameters tab](/results/parameters)
(["Sequencing Type" parameter](/settings/settings/alignment_settings#sequencing-type-detection):
the "WGS" option corresponds to either type "WGS" or "Low-pass WGS", the "Targeted selection" option corresponds to
either type "Panel" or "WES").
- ALIAS is a synonymous name for the sample, which can be entered by clicking on the value field.
- CAPTURE KIT is a targeted panel used in targeted selection. Determined automatically by Genomenal, or by user while
adding sample set. If the capture kit is defined incorrectly, it can be changed
on [the Parameters tab](/results/parameters). You can use both built-in capture kits (standard panels often used in
sequencing) and custom capture kits uploaded by the user (on the "Capture kits" page of the "Settings" page block).
...
as it occurred at the time the sample was taken for sequencing.
...
then it has a status like <img src={require('/img/results/complete_metrics.png').default} width="300"/> indicating
...
Then you will see this stage section on [the "Workflow details" tab](/results/workflow_details/workflow-details).
