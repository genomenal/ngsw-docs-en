...
blocks. The potential findings section will not appear on the sample page if no report, that uses potential findings,
...
- READS TYPE - the type of nucleic acid (DNA, RNA, or UNKNOWN) whose sequence was obtained during sample sequencing.
The reads type can be assigned in two ways:
(a) automatically - as a result of the built-in classifier ("Determine reads type" task);
(b) manually - via the settings preset selected during sample upload.
If the assigned reads type is incorrect, it can be manually changed by clicking on the current value and
selecting the appropriate type from the dropdown list.
