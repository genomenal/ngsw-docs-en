...
by [the corresponding setting](/settings/settings/variant-discovery-settings#minimal-bin-size) for WGS,
...
You can pin the needed variations. Pinned variations are not affected
...
in IGV by clicking on the button <img src={require('/img/vv/back_to_cnvv.png').default} width="130"/>.
...
in the variation row and select the needed option in the dialog that appears:
