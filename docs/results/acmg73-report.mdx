...
:::caution
ACMG secondary findings report generation has its limitations due to variant annotation using ClinVar.
These limitations are described [below](/results/acmg73-report#acmg73-limitations).
:::
...
Only those variants that are **pathogenic** or **likely pathogenic** according
to [ClinVar](https://preview.ncbi.nlm.nih.gov/clinvar/) are included in the report. In the case of variants
from the VCF file with a single sample, only those variants whose sites has at least one alternative allele
(i.e. variants with the genotype 1/1, 1|1, 0/1, 1/0, 0|1 etc.) will be included in the report.

The report includes two sections:
1. The most important variants for interpretation for which the following conditions are met:
...
phenotype that is listed in [the ACMG SF list](https://www.ncbi.nlm.nih.gov/clinvar/docs/acmg/) for
the gene in which the variant is located;
- The allele frequency in [GnomAD](/results/main/snvs-indels/advanced-filtering#gnomad) is below 0.25 or empty;
...
for the gene in which the variant is located. These conditions may be associated with:
   - the variant pathogenicity (only variants that are pathogenic or likely pathogenic according
   to [ClinVar](https://preview.ncbi.nlm.nih.gov/clinvar/) are generally included in the report);
   - type of inheritance;
   - the number of variants discovered in the sample that fit the conditions from the ACMG SF list
   (for example, for some variants with an autosomal recessive inheritance to be included in the report,
   at least two such variants must be discovered in the sample);
   - the variant origin;
...
2. Variants that require further analysis for which the following conditions are met:
- The variant is located in a gene from [the ACMG SF list](https://www.ncbi.nlm.nih.gov/clinvar/docs/acmg/);
- The allele frequency in [GnomAD](/results/main/snvs-indels/advanced-filtering#gnomad) is below 0.25 or empty;
- The variant site has at least one alternative allele (i.e. has the genotype 1/1, 1|1, 0/1, 1/0, 0|1, etc.)
in the case of a VCF file with a single sample;
- The variant is either not associated according to [ClinVar](https://preview.ncbi.nlm.nih.gov/clinvar/) with
a phenotype that is listed in [the ACMG SF list](https://www.ncbi.nlm.nih.gov/clinvar/docs/acmg/) for the gene
in which the variant is located, or it does not meet other conditions specified
...
### Report limitations in ClinVar as a database source {#acmg73-limitations}

ACMG 73 secondary findings report generation has limitations
of [phenotypic](/results/acmg73-report#phenotypic-data)
and [pathogenicity](/results/acmg73-report#pathogenicity-data) data
...
3. The variant must meet any additional conditions specified in the list of secondary findings,
which often include pathogenicity or likely pathogenicity of the variant.
...
ACMG Secondary Findings Report by Genomenal includes two groups of variants: those for which the phenotypes
...
*The first section of the report may include variants:*
...
*The second section of the report will include the following variants:*
...
*Therefore, the interpretation of the variants in the second section of the report must be approached with caution.*
