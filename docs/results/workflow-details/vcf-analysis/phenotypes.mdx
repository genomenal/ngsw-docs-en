...
After the "Unify SNVs/Indels" task has been successfully completed, the phenotype prediction pipeline can be launched
for non-tumor samples if at least one of the following tasks was included in the analysis
...
Please note that to include phenotype prediction in the workflow of a sample uploaded in VCF or GT format, you
...
The "Phenotype prediction" stage may include the following tasks:
...
4. **Phenotypes prediction** (oligogenic risk calculation)
if [the "*Run phenotypes prediction*" parameter](/settings/settings/variant-discovery-settings#enable-phenotype-prediction)
is enabled:
