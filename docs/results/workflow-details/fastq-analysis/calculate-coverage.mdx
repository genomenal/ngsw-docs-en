...
of interest (genes, transcripts). The coverage calculation results are located
...
The subtask is performed only if [gene coverage analysis](/settings/settings/alignment-settings#gene-coverage-analysis)
is enabled for the sample.
- [bedtools coverage](https://bedtools.readthedocs.io/en/latest/content/tools/coverage.html)
...
as a summary histogram for all exons. The subtask is performed only
if [gene coverage analysis](/settings/settings/alignment-settings#gene-coverage-analysis) is enabled for the sample.

The gene coverage file is generated only
if [gene coverage analysis](/settings/settings/alignment-settings#gene-coverage-analysis) is enabled for the sample.
It can be downloaded in the "*Result files*" section
...
computes the depth and breadth of coverage of sample read alignments on exons
from [Ensembl](https://www.ensembl.org/index.html) and reports a histogram of coverage for each exon as well
as a summary histogram for all exons.<br></br>
The subtask is performed only
if [gene coverage analysis](/settings/settings/alignment-settings#gene-coverage-analysis) is enabled for the sample.
As a result, a gene coverage file is generated, which can be downloaded in the "*Result files*" section
in the "*Calculate coverage statistics*" task details ("*Download Gene coverage TSV*").
You can also open it in Google Sheets.
...
computes histograms of aligned sample sequences coverage for the reference genome.
...
2. **Save coverage** task is saving of gene and transcript coverage. The task is performed only
if [saving detailed per-gene coverage](/settings/settings/alignment-settings#save-detailed-gene-coverage) is enabled
for the sample.
