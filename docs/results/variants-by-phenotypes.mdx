---
sidebar_position: 3
sidebar_label: ClinVar Phenotypes Report
---

# ClinVar Phenotypes Report

The report includes ***conditions*** (ClinVar phenotypes) that have the risk of developing in the patient
due to pathogenic or likely pathogenic variants discovered in patient sample and associated with these
conditions, and ***recommendations*** that can prevent these conditions.
...
- A list of conditions (ClinVar phenotypes) that have the risk of developing in the patient or patient's closest
...
3. The total allele frequency in gnomAD is above the frequency threshold for extremely rare pathogenic variants
...
in [gnomAD](https://gnomad.broadinstitute.org/) database). If there is no data on the frequency in the
population, then the probability is taken as 0.001.<br></br>
...
5. **gnomAD AF** is the total allele frequency in [gnomAD](https://gnomad.broadinstitute.org/) database.
