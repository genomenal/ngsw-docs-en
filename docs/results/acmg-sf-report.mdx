...
Below the variant row, there can be phenotypes with pathogenic or likely pathogenic clinical significance that the
variant is associated with according to [ClinVar](https://preview.ncbi.nlm.nih.gov/clinvar/) and that are listed
in [the ACMG SF list](https://www.ncbi.nlm.nih.gov/clinvar/docs/acmg/) for the gene in which the variant is located.
