...
**LIST 1**: The most important variants for interpretation for which the following conditions are met:
...
**LIST 2**: Variants that require further analysis for which the following conditions are met:
- The variant is located in a gene from [the ACMG SF list](https://www.ncbi.nlm.nih.gov/clinvar/docs/acmg/);
...
- The variant site has at least one alternative allele (i.e. has the genotype 1/1, 1|1, 0/1, 1/0, 0|1, etc.)
in the case of a VCF file with a single sample;
- The variant does not meet at least one of the conditions in List 1:
