...
(added [by default](/settings/profile#basic-mode-filters)). In order to filter the variants by the run samples
...
In order to filter the variants by the interpretation status of run samples
